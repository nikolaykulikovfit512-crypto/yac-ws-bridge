...
1. **Adapter** — соберите (см. [Adapter / Build](#build)) и запустите на удалённом сервере, желательно рядом с тем, через что вы в итоге проксируете (Dante / XRay-core / и т.п.). HTTP-порт `/conn-ids` должен быть доступен извне — Serverless Function ходит на него при холодном старте.
2. **Cloud Function** — задеплойте [`bridge-cloud/`](bridge-cloud/) в Yandex Cloud Functions и привяжите к API Gateway. В env-переменной `ADAPTER_CONN_IDS_URL` укажите `https://<сервер>:<порт>/conn-ids` вашего адаптера, и используйте один и тот же `AUTH_TOKEN` (общий секрет) во всех трёх компонентах. Не забудьте обфусцировать JS перед заливкой (см. предупреждение выше).
...
- На телефоне: ставите MAUI-клиент (это приложение, BTF) и [v2rayNG](https://github.com/2dust/v2rayNG). В v2rayNG включаете per-app проксирование и выбираете именно те приложения, которые хотите гонять через туннель (например, только Chrome и Telegram — это важно, чтобы v2rayNG НЕ пытался завернуть собственный upstream-трафик BTF, иначе получится цикл). Создаёте новый outbound-профиль типа SOCKS (или VLESS) и указываете адрес `127.0.0.1:5080`. Сначала запускаете BTF и подключаетесь к серверу, потом включаете v2rayNG.
